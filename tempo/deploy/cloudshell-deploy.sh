...
rm -f "$KEY" "${KEY}.pub" 2>/dev/null
ssh-keygen -t ed25519 -f "$KEY" -N "" -q || planB "Couldn't generate a temp key in Cloud Shell" <<'EOF'
ssh-keygen failed — very unusual in Cloud Shell. Try: rm -f ~/.ssh/tempo_deploy_*
